		...
		if numUsers > 0:
			...
			for ip in dictOnline.keys():
				...
				if numUsers == 0:
					...
					pythonBan.banPhp(idUserIp, 1, 3600, "Verificador de seguranca excluido")
					deletePorIp(ip)
					...
					if verificador == "close": 
						...
						pythonBan.banPhp(idUserIp, 1, 3600, "Verificador de seguranca fechado")
						deletePorIp(ip)
						adicionarTabela.adicionarNaTabela(idUserIp, dictOnline.get(ip)[1], "Verificador de seguranca fechado")
					
					if hackername != "":
						
						pythonBan.banPhp(idUserIp, 1, 3600, "Uso de Hack")
						deletePorIp(ip)
